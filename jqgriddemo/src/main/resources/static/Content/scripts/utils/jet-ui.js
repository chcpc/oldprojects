...
initJGGridselectView = function(Jggird){
    ...
    for (i = 0; i < colmodel.length; i++) {
        ...
        var s = colmodel[i];
        if (s.label) {// 由label
            if(s.hidden != true || s.isIndex == true ){//非隐藏，有index(该列来源于后台)
                if(s.index != undefined){// index不是undefined
    ...
    $("#btn_Search").click(function () {
        // 获取选择条件中data-value的值，并赋值给whereName
        ...
        if($("#txt_Keyword").attr("selectType") == "select"){
            ...
            // whereName += " between '"+$("#txt_Keyword").val()+"' and '"+$("#txt_Keyword2").val();
            // whereName += ":{begin:'"+$("#txt_Keyword").val()+"'";
            // whereName += ",end:'"+$("#txt_Keyword2").val()+"'}";
            whereValue = $("#txt_Keyword").val()+","+$("#txt_Keyword2").val();
        }else{
